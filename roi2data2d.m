...
## -*- texinfo -*-
## @anchor{roi2data2d}
## @deftypefn {Function File} {[@var{Xroi}, @var{abroi}] =} roi2data2d (@var{X}, @var{ab}, @var{roi})
## Concatenates vectorized two-dimensional spectral data inside regions
## of interest into a data matrix.
## @end deftypefn

function [Xroi, abroi] = roi2data2d (X, ab, roi)
  % check the number of input arguments.
  if (nargin != 3 || nargout != 2 || !iscell(X) || !isvector(ab))
  ...
  if (!ismatrix(roi) || columns(roi) != 4 || rows(roi) < 1)
    ...
    error('roi2data2d: invalid region of interest matrix');
  end
  ...
  if (length(ab{1}) != Kb || length(ab{2}) != Ka)
    ...
    error('roi2data2d: variable count and abscissa length do not match');
  end
  ...
  for idx = 1 : rows(v)
    ...
    for k = 1 : N
      ...
      Xk = X{k};
      Xi = [Xi; reshape(Xk(j1 : j2, i1 : i2), 1, nk)];
    end
