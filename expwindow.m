...
## -*- texinfo -*-
## @anchor{expwindow}
## @deftypefn {Function File} {@var{w} =} expwindow (@var{t}, @var{lb})
## Calculate window coefficients for exponential windowing of a signal. The
...
function w = expwindow (t, lb)
  ...
  if (nargin >= 2 && !isempty(lb))
    ...
    if (!isscalar(lb) || !isreal(lb))
      ...
      error('expwindow: line-broadening factor must be a real scalar');
    end
  else
