...
## @end tex
## @deftypefn {Function File} {@var{Xc} =} snone (@var{X})
## @deftypefnx {Function File} {[@var{Xc}, @var{mu}] =} snone (@var{X})
## @deftypefnx {Function File} {[@var{Xc}, @var{mu}, @var{s}] =} snone (@var{X})
## @deftypefnx {Function File} {@dots{} =} snone (@var{X}, @var{w})
## Performs only mean-centering, but no scaling. An optional weighting vector
...
## $$ \tilde{x}_{ik} = { x_{ik} - \bar{x}_k \over w_k } $$
## @end tex
## @end deftypefn

function [Xnew, mu, s] = snone (X, w)
